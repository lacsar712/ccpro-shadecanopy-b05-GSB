...
def validate_target_zone_not_paused(attrs, instance, resource_label):
    """新建时目标分区暂停 -> 409；更新时不换区允许，换到已暂停区 -> 409。"""
    zone = attrs.get("zone")
    if instance is not None and (zone is None or zone.pk == instance.zone_id):
        return attrs
    if zone is not None and zone.is_paused:
        if instance is None:
            raise ZonePausedConflict(
                f"分区 {zone.zone_code} 已暂停，暂停期间禁止新建{resource_label}"
            )
        raise ZonePausedConflict(
            f"分区 {zone.zone_code} 已暂停，不得将记录改挂到已暂停分区"
        )
    return attrs
...
class ClimateLogSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        return validate_target_zone_not_paused(attrs, self.instance, "气候记录")
...
class IrrigationCycleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        return validate_target_zone_not_paused(attrs, self.instance, "轮灌")
